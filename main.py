...
async def get_postal_codes_by_bounding_box(min_lat: float, min_lon: float, max_lat: float, max_lon: float,db=Depends(db.connection)):
    #record = await db.fetch ("select pc.code as postal_code, pc.name as postal_code_name, pc.lat as lat, pc.lon as lon from postal_codes as pc where pc.lat >= $1 and pc.lat <= $2 and pc.lon >= $3 and pc.lon <= $4", min_lat, max_lat, min_lon, max_lon)
